...
def generate_job_submission_script(num_mpi_processes, node_hours, graph_name):
    srcgrp = "small-s1"
    if int(node_hours) > 1:
        srcgrp = "small-s2"
    ...
    script += "dir_stdout=./log/${PJM_MPI_PROC}proc/"

    script += "\n"
    script += "tcmalloc_path=/home/ra000012/a04083/gnn/gnn/pytorch/scripts/fujitsu/lib/libtcmalloc.so\n"
    script += "date\n"
    script += "LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}/stdout -stderr-proc ${dir_stdout}/stderr python ../../train.py --config=../../config/${graph_name}.yaml --num_bits=32\n"
    # script += "LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}/ori/stdout -stderr-proc ${dir_stdout}/ori/stderr python ../../../examples/graphsage/train.py --config=../../../config/fugaku/${graph_name}.yaml --num_bits=32\n"
    # script += "LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}/ori+pre/stdout -stderr-proc ${dir_stdout}/ori+pre/stderr python ../../../examples/graphsage/train.py --config=../../../config/fugaku/${graph_name}.yaml --num_bits=32 --is_pre_delay=true\n"
    # script += "LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}/ori+int2/stdout -stderr-proc ${dir_stdout}/ori+int2/stderr python ../../../examples/graphsage/train.py --config=../../../config/fugaku/${graph_name}.yaml --num_bits=2\n"
    # script += "LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}/ori+pre+int2/stdout -stderr-proc ${dir_stdout}/ori+pre+int2/stderr python ../../../examples/graphsage/train.py --config=../../../config/fugaku/${graph_name}.yaml --num_bits=2 --is_pre_delay=true\n"
    script += "date\n"
...
output_dir = os.path.join("./", graph_name)
...
if graph_name == "ogbn-products":
    ...
    num_mpi_processes_list = [4, 8, 16, 32, 64, 128, 256, 512, 1024]
    node_hours_list = ["1", "1", "1", "1", "1", "1", "1", "1", "1"]
elif graph_name == "ogbn-papers100M":
    num_mpi_processes_list = [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384]
    node_hours_list = ["12", "10", "6", "3", "1", "1", "1", "1", "1", "1", "1", "1"]
